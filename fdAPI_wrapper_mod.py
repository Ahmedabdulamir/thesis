...
# --- Installation dir ---
FD_root = 'c:\\program files\\strusoft\\fem-design 19\\fd3dstruct'
...
class material:
    def __init__(self, type='', creep='', shrinkage='' , poisson=''):               #Modified
        self.type = type
        self.creep = creep
...
def finish(filename):
    ...
    # clear global
    xml_complexsection.clear()
    xml_bars.clear()
    xml_shells.clear()
    xml_pointsupports.clear()
    xml_linesupports.clear()
    xml_surfacesupports.clear()
    xml_pointloads.clear()
    xml_lineloads.clear()
    xml_surfaceloads.clear()
    xml_loadcases.clear()
    xml_loadcombs.clear()
...
def addMaterial(i_material):
    ...
    if i_material.type in ('C12/15', 'C16/20', 'C20/25', 'C25/30', 'C28/35', 'C30/37', 'C32/40', 'C35/45', 'C40/50', 'C45/55', 'C50/60', 'C54/65', 'C55/67', 'C58/70', 'C60/75', 'C70/85', 'C80/95', 'C90/105'):
        ...
        mat_add[0].attrib['nu'] = str(i_material.poisson)
...
def runFD(analysis, save, close, design, filename, batchfile, exportfile):            #Batchfile and filename must be string lists  
    ...
    if design == 'steel' or design == 'timber' or design == 'RC':
        ...
        run_design = ET.SubElement(run_cmdcalc, "design")
        cmax = ET.SubElement(run_design, "cmax")
        autodesign = ET.SubElement(run_design, "autodesign")
        autodesign.text= "false"
        check = ET.SubElement(run_design, "check")
        check.text = "true"
    ...
    for i in range (0, len(exportfile)):
        ET.SubElement(run_root, 'cmdlistgen', command='$ MODULECOM LISTGEN', bscfile=dir_path + '\\' + batchfile[i], outfile=dir_path + '\\' + exportfile[i])
    if save == True:
...
def addPlate(material_ID, thickness1, thickness2, release, point1, point2, alignment, meshSize):     #Modified
    shellName = ID('plate')
    ...
    add_plate_child = ET.SubElement(add_plate, 'slab_part', guid=genGUID(), last_change=genUTC(), action='added', name=(shellName + '.1'), 
    complex_material=material_ID, alignment=alignment, align_offset='0', ortho_alfa='0', ortho_ratio='1', mesh_size=meshSize)
...
def addPlateComplex(material_ID, thickness, release, *points, alignment ):
    shellName = ID('plate')
    add_plate = ET.SubElement(xml_root.find('entities'), 'slab', name=shellName, guid=genGUID(), last_change=genUTC(), action='added', type='plate')
    add_plate_child = ET.SubElement(add_plate, 'slab_part', guid=genGUID(), last_change=genUTC(), action='added', name=(shellName + '.1'), complex_material=material_ID, alignment=alignment, align_offset='0', ortho_alfa='0', ortho_ratio='1')
    add_plate_grandchild = ET.SubElement(add_plate_child, 'contour')
...
def addPointSupport(point, type):
    ...
    if type == 'fixed':
        ...
        ET.SubElement(add_pointsupport_grandchild, 'motions', x_neg='10000000000', x_pos='10000000000', y_neg='10000000000', y_pos='10000000000', z_neg='0', z_pos='0')
        ET.SubElement(add_pointsupport_grandchild, 'rotations', x_neg='0', x_pos='0', y_neg='0', y_pos='0', z_neg='0', z_pos='0')
    elif type == 'zpinned':
        ET.SubElement(add_pointsupport_grandchild, 'motions', x_neg='0', x_pos='0', y_neg='0', y_pos='0', z_neg='10000000000', z_pos='10000000000')
        ET.SubElement(add_pointsupport_grandchild, 'rotations', x_neg='0', x_pos='0', y_neg='0', y_pos='0', z_neg='0', z_pos='0')
    else:
        ET.SubElement(add_pointsupport_grandchild, 'motions', x_neg='10000000000', x_pos='10000000000', y_neg='10000000000', y_pos='10000000000', z_neg='10000000000', z_pos='10000000000')
        ET.SubElement(add_pointsupport_grandchild, 'rotations', x_neg='0', x_pos='0', y_neg='0', y_pos='0', z_neg='0', z_pos='0')
